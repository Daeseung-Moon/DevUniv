...
class TextField extends StatefulWidget {
  ...
  override createState(): TextFieldState {
...
class TextFieldState extends State<TextField> {
  ...
  #selectionUI: SelectionSegment[] | null = null;
  ...
  #isComposing = false;
  #lineInfo: LineInfo[] = [];
  #currentCharUI?: CurrentCharUI | null = null;
    ...
    for (let i = startLineIndex; i <= endLineIndex; i++) {
      ...
      const startX =
        segmentStart > 0 && segmentStart <= spanBoxes.length
          ? spanBoxes[segmentStart - 1].offset.x + spanBoxes[segmentStart - 1].size.width
          : spanBoxes[0]?.offset.x || 0;

      const endX =
        segmentEnd > 0 && segmentEnd <= spanBoxes.length
          ? spanBoxes[segmentEnd - 1].offset.x + spanBoxes[segmentEnd - 1].size.width
  ...
  #getCharIndexFromMouseEvent = (e: MouseEvent): number => {
    ...
    const [x, y] = [e.clientX - this.#textFieldPosition.x, e.clientY - this.#textFieldPosition.y];
              ...
              child: ConstraintsTransformBox({
                ...
                constraintsTransform: (constraints) => {
                  return new Constraints({
                    minHeight: Math.max(this.widget?.height ?? 0, constraints.minHeight),
...
class CaretState extends State<Caret> {
  ...
  override didUpdateWidget(oldWidget: Caret): void {
...
class NativeInput {
  ...
  #createElement() {
    ...
    el.addEventListener('input', (e: Event) => {
       ...
       */
      if (inputEvent.isComposing && this.value.endsWith(' ')) {
        this.#setComposing(false);
        return;
      }

      this.#setComposing(inputEvent.isComposing);
    });
